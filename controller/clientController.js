...
client.clientList = async (req, res) => {
    try {
        var user=req.user
        var client = await knex.select("name", "mobileno").from("invoice_details").where("invoice_status", 1).where("created_by",user.id)
        if (client.length === 0) {
...
client.InvoiceListByClient = async (req, res) => {
    try {
        var user=req.user
        const clinetinvoice = await knex.select("invoice_link").from("invoice_details").where("mobileno", req.query.mobileno).where("created_by",user.id)
